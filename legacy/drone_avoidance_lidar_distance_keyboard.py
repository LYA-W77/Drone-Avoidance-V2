...
- 激光雷达为主，距离传感器为辅（冗余容错）
- 支持三种模式：AUTO（自动避障）、MANUAL（点动键盘控制）、EMERGENCY_STOP（紧急悬停）
- 按 P 切换 AUTO <-> MANUAL，按 空格/ESC 紧急停止，按 R 恢复 AUTO
- 手动模式：W(前进1秒) S(后退0.5秒) A(左转15°) D(右转15°)
- 注意：按键前请点击黑色终端窗口确保焦点
"""
...
class ObstacleAvoidanceDrone:
    ...
    def reset_camera_follow(self):
        ...
        try:
            ...
            cam_pos = airsim.Vector3r(pos.x_val + back_x, pos.y_val + back_y, pos.z_val - 3.0)
    ...
    def run(self, max_time=360):
        try:
            ...
            print("=" * 40)
            print("键盘控制说明（请保持此黑框为焦点）：")
            print("  P 键切换 AUTO/MANUAL")
            print("  空格/ESC 紧急停止，R 恢复")
            print("  手动模式下：W(前进1秒) S(后退0.5秒) A(左转15°) D(右转15°)")
            print("=" * 40)
            ...
            while time.time() - start_time < max_time:
                ...
                if msvcrt.kbhit():
                    ...
                    # ---- 恢复 ----
                    if key_char == 'r' and self.mode == FlightMode.EMERGENCY_STOP:
                        self.mode = FlightMode.AUTO
                        print("✅ 已恢复自动模式")
                        continue

                    # ---- 手动控制（仅在 MANUAL 模式下有效） ----
                    if self.mode == FlightMode.MANUAL:
                        if key_char == 'w':
                            ...
                            self.client.rotateToYawAsync((yaw_deg + 15) % 360, timeout_sec=1).join()
                        elif key_char == 'd':
                            print("↻ 右转 15°")
                            state = self.client.getMultirotorState()
                            yaw_deg = math.degrees(
