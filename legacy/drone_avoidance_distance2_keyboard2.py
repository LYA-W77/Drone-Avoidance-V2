...
class ObstacleAvoidanceDrone:
    def __init__(self, verbose=True):
        ...
        # ===== 手动控制平滑参数 =====
        self.manual_vx = 0.0          # 当前实际前向速度（平滑后）
        self.manual_vy = 0.0          # 当前实际侧向速度（平滑后）
        self.manual_speed_max = 1.5   # 最大速度（原3.0减半，更稳）
        self.manual_smooth_factor = 0.15  # 平滑系数（越小手感越柔顺）
    ...
    def get_manual_velocity(self):
        """获取键盘 WASD 映射，输出平滑后的机体系速度 (vx, vy)"""
        # 目标速度（由按键决定）
        target_vx = 0.0
        target_vy = 0.0
        ...
        if self.keyboard_available:
            kb = self.keyboard_module
            try:
                if kb.is_pressed('w'): target_vx = speed
                if kb.is_pressed('s'): target_vx = -speed
                if kb.is_pressed('a'): target_vy = speed   # 左移
                if kb.is_pressed('d'): target_vy = -speed  # 右移
            except:
                pass

        # 指数平滑（低通滤波）
        alpha = self.manual_smooth_factor
        self.manual_vx += (target_vx - self.manual_vx) * alpha
        self.manual_vy += (target_vy - self.manual_vy) * alpha
